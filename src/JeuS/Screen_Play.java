...
import Cuboidbox.Cuboid;
import Cuboidbox.CuboidActive;
import Cuboidbox.CuboidDraw;
import Cuboidbox.CuboidNormal;
import Cuboidbox.CuboidO;
import Cuboidbox.CuboidPlayer;
import Cuboidbox.CuboidX;
import Cuboidbox.Level;
import Cuboidbox.ManagerLevel;
import Cuboidbox.RSSound;
import Cuboidbox.Skybox;
import Cuboidbox.CuboidPlayer.Action;
import framework.game;
import framework.iState;

public class Screen_Play extends iState{
	Skybox mSky;
	CuboidNormal[][] mArrCube;
	int X,Z;
	CuboidPlayer mCubePlayer;
	float mCamPos[];
	float OldPoint[];
...
//				gl.glTranslatef(i, 0, j);
	...
	private int CheckCubeVSBoard (CuboidPlayer _Player,CuboidNormal[][] _Cubes,int _x,int _z){
		int result= 0;
	...
	public void initLevel (int _level){
		...
		X= 20;
		Z= 10;
		mCubePlayer = new CuboidPlayer(Global.get().LoadTexture(Global.IMG_PLANE));
		mArrCube=new CuboidNormal[X][Z];
		switch (_level) {
		case 1:
